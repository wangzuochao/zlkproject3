...
$(".comment-reply-link").click(function () {
    ...
    $("input[name=commentId]").attr("value", commentId);
    $("input[name=userId]").attr("value", null);
    $("#comment").attr("placeholder", "@ " + authorName)
})

$("#cancel-comment-reply-link").click(function () {
    $("#cancel-comment-reply-link").hide();
    $("input[name=commentId]").attr("value", null);
    $("input[name=userId]").attr("value", "");
    $("#reply-title-word").html("发表评论");
})
//ajax提交评论信息
$("#comment_form").submit(function () {
    var commentArticle = $("#comment_form").serialize();
...
//踩+1
function increaseStampCount() {
    if ($.cookie("stampId") != articleId || $.cookie("stampId") == null) {
        $.ajax({
            async: false,
            type: "POST",
            url: "/article/stamp/" + articleId,
            dataType: 'json',
            contentType: 'application/json',
            success: function (data) {
                $(".stampCount").html(data);
                $.cookie(
                    "stampId",
                    articleId,//需要cookie写入的业务
                    {
